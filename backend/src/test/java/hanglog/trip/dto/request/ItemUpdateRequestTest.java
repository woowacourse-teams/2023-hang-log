...
public class ItemUpdateRequestTest {
    ...
    @DisplayName("Spot인 아이템의 장소의 isPlaceUpdated가 true일 때, 장소 정보가 Null이면 예외가 발생한다.")
    @Test
    void validatePlaceWhenSpotAndPlaceUpdated() {
        assertThatThrownBy(() -> new ItemUpdateRequest(
                true,
                "title",
                1.0,
                "memo",
    ...
    @DisplayName("NonSpot인 아이템의 isPlaceUpdated가 true라면, 예외가 발생한다.")
    @Test
    void validateIsPlaceUpdatedWhenNonSpot() {
        assertThatThrownBy(() -> new ItemUpdateRequest(
                false,
                "title",
                1.0,
                ...
                .isInstanceOf(BadRequestException.class)
                .hasMessage(INVALID_IS_PLACE_UPDATED_WHEN_NON_SPOT.getMessage())
                .extracting("code")
                .isEqualTo(INVALID_IS_PLACE_UPDATED_WHEN_NON_SPOT.getCode());
    }

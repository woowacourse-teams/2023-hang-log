  ...
  return (
    <Modal isOpen={isOpen} closeModal={() => {}} hasCloseButton>
      <form css={formStyling}>
        <CitySearchBar initialCityTags={cities} setCityData={() => {}} />
        <DateInput initialDateRange={{ start: startDate, end: endDate }} setDateData={() => {}} />
        <Input label="여행 제목" value={title} required />
        <Input label="여행 설명" value={description ?? ''} />
        <ImageUploadInput
          label="대표 이미지 업로드"
          imageUrls={imageUrl === null ? imageUrl : [imageUrl]}
